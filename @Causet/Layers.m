function s = Layers( obj, list, k, varargin )
%LAYERS    Returns the events in the layers k(1) to k(end) of the events 
%   list. 
% 
% Arguments:
        ...
        if k_directions( ki ) >= 0 % future (or present) layer:
            for b = obj.FutureOf( list, 'origins', true )
                ...
                for a = obj.PastOf( b, 'partof', list )
                    n = max( n, obj.Interval( a, b, 'return', 'card' ) - 1 );
                end
            ...
            for a = obj.PastOf( list, 'origins', true )
                ...
                for b = obj.FutureOf( a, 'partof', list )
                    n = min( n, -obj.Interval( a, b, 'return', 'card' ) + 1 );
                end
                if ( n >= k_min( ki ) ) && ( n <= k_max( ki ) )
                    s( a ) = true;
                end

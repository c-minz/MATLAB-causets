...
%                     to as the Myrheim-Meyer dimension.
% 
...
%     year = {2013}
% 
% Warning: This is function is an incomplete state and needs to be
% remastered.
    ...
    persistent accuracy dim dimlen coeffflat coeffcurved
    dim = [];
    if isempty( dim ) || accuracy ~= newaccuracy 
        ...
        temp = sqrt( abs( coeffcurved( 2, : ) .* coeffcurved( 4, : ) ) );
        for i = 1 : 4
            coeffcurved( i, : ) = coeffcurved( i, : ) ./ temp;
        end
    end
    ...
    elseif length( chains ) == 4 % curved estimator
        ...
        % a step by step addition is necessary.
        estimators = coeffcurved( 1, : ) / chains( 4 ) .* chains( 1 )^4;
        estimators = estimators + coeffcurved( 2, : ) / chains( 4 ) .* chains( 2 )^2;
        estimators = estimators + coeffcurved( 3, : ) / chains( 4 ) .* chains( 3 )^( 4 / 3 );
        estimators = estimators + coeffcurved( 4, : );
    else
        estimators = 0;
    end
    %% return first n results:
    plot( dim, max( -1, min( 1, estimators ) ) );
    ylim( [ -1, 1 ] );
    estimators = abs( estimators );
    estimators_sel = ( estimators == min( estimators ) ); % first closest to zero
    MMdim = dim( estimators_sel );
%     if length( MMdim ) > 1 % multiple solutions
%         return % all solutions
%     end
%     % single solution, search for more:
%     while true
%         estimators_sel = estimators_sel ...
%             | ( estimators == min( estimators ) );
%         estimators( estimators_sel ) = NaN;
%     end
end
